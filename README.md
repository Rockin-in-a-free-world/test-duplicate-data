...
**Webpack Alias Configuration**: Because MetaMask's documentation uses absolute paths like `/services/reference/_partials/...` (they don't put all docs in a `docs/` folder), a webpack alias is configured in `docusaurus.config.js` to map `/services/` to `docs/services/`. This allows the absolute import paths in the downloaded files to resolve correctly during the build.

### Import script

The import script is located at:
- **`scripts/get-remote.js`** - Custom script that handles downloading remote content

#### What the import script does

### 1. Downloads Remote Content

Downloads all configured content sources from MetaMask docs using the plugin's. The configuration is managed in docusaurus.config.js at the root of this repo.
