// import CreditCost from '@site/src/components/CreditCost/CreditCostPrice.js'; // Component not available in this project

Polling method for a filter, which returns an array of logs which occurred since the last poll. {/* <CreditCost network="evm_filter" method="eth_getFilterChanges" />... - Component not available */}

Filter must be created by calling either [`eth_newFilter`](../eth_newfilter) or [`eth_newBlockFilter`](../eth_newblockfilter).
